...
const CLIENT_ID = process.env.TAU_GOOGLE_GEMINI_CLIENT_ID ?? "";
const CLIENT_SECRET = process.env.TAU_GOOGLE_GEMINI_CLIENT_SECRET ?? "";
...
function requireClientCredentials(): void {
	if (!CLIENT_ID || !CLIENT_SECRET) {
		throw new Error(
			"Google Gemini OAuth requires TAU_GOOGLE_GEMINI_CLIENT_ID and TAU_GOOGLE_GEMINI_CLIENT_SECRET",
		);
	}
}
