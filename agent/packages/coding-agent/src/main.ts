...
/**
 * Load solver instructions from AGENTS.md in the agent workspace root.
 * In Docker the compiled cli.js lives at packages/coding-agent/dist/,
 * so the workspace root is three levels up.
 */
const _agentWorkspaceRoot = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "..");

function _loadSolverInstructions(): string | undefined {
	try {
		const agentsPath = join(_agentWorkspaceRoot, "AGENTS.md");
...
export async function main(args: string[]) {
	...
	// In non-interactive (validator) mode, auto-inject solver instructions from AGENTS.md
	// so the LLM receives scoring-optimized guidance regardless of the target repo's contents.
	if (!isInteractive && !parsed.systemPrompt && !parsed.appendSystemPrompt) {
		const solverInstructions = _loadSolverInstructions();
		if (solverInstructions) {
			parsed.appendSystemPrompt = solverInstructions;
		}
	}
